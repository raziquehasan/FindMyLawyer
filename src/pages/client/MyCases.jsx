...
import {
  ...
  MessageSquare, CreditCard, Clock, AlertCircle, Star
} from "lucide-react";
    ...
    try {
      ...
      if (stored) {
        try {
          ...
          // ✅ Deduplicate by bookingId — keeps only the first occurrence
          const seen = new Set();
          ...
          // ✅ Also write deduped list back to fix localStorage
      ...
      console.error("MyCases load error:", err);
      navigate("/");
    }
  }, [navigate]);
    ...
    <div className="min-h-screen bg-gray-50 pb-24 font-sans">
      ...
      <main className="max-w-3xl mx-auto px-3 sm:px-4 pt-6 pb-4">
        <div className="mb-5">
          ...
          <p className="text-xs sm:text-sm text-gray-500 mt-0.5">Track and manage your legal consultations</p>
        </div>
        ...
        <div className="max-w-4xl mx-auto">
          <div className="flex items-center justify-between px-4 sm:px-8 py-2.5">
            <div className="flex items-center gap-4 sm:gap-8">
              <button onClick={() => navigate("/profile")} className="flex flex-col items-center gap-1 text-gray-400 hover:text-gray-900 transition-all min-w-[60px]">
                <User size={22} />
                <span className="text-[10px] font-medium">Profile</span>
              </button>
              <button onClick={() => navigate("/support-chat")} className="flex flex-col items-center gap-1 text-gray-400 hover:text-blue-600 transition-all min-w-[60px]">
            ...
            <button
              onClick={() => navigate("/client-dashboard/request")}
              className="flex flex-col items-center gap-1 px-6 py-2.5 bg-gray-900 text-white rounded-full shadow-2xl hover:bg-gray-800 hover:scale-105 active:scale-95 transition-all -mt-8"
            >
              <PlusCircle size={28} />
              <span className="text-[9px] font-semibold whitespace-nowrap">New Consultation</span>
            </button>

            <div className="flex items-center gap-4 sm:gap-8">
              <button onClick={() => navigate("/client-dashboard/lawyers")} className="flex flex-col items-center gap-1 text-gray-400 hover:text-gray-900 transition-all min-w-[60px]">
                <Shield size={22} />
                <span className="text-[10px] font-medium">Lawyers</span>
              </button>
              <button onClick={() => navigate("/client-dashboard")} className="flex flex-col items-center gap-1 text-gray-400 hover:text-gray-900 transition-all min-w-[60px]">
                <Briefcase size={22} />
